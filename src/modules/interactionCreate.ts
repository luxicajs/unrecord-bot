...
export default async (interaction: Interaction) => {
  ...
  if (
    ...
    interaction.customId.startsWith("reportModal-")
  ) {
    const msgId = interaction.customId.replace("reportModal-", "");

    if (!interaction.channel)
      return console.error(`Fatal error. Channel not found in cache.`);
    ...
    const container = new ContainerBuilder()
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`> :people_hugging:  Author:
...
${interaction.fields.getTextInputValue("context") || "No context included"}`),
        ...
        new SectionBuilder()
          .addTextDisplayComponents(
            new TextDisplayBuilder().setContent(
              `> :cinema:  Recorded message content:\n${msg.data.content || "No content"}`,
            ),
          )
          .setButtonAccessory(
            new ButtonBuilder()
              .setURL(
                `https://discord.com/channels/${interaction.guildId}/${interaction.channelId}/${msg.data.id}`,
              )
    ...
    const successContainer = new ContainerBuilder()
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `-# By submitting this report you understand that you are doing this with good faith in accordance with the server rules, and any abuse of this system will result in a permanent removal from this server. Staff members reserve the right to decide if they take action against the user or spare them.`,
        ),
      )
      .setAccentColor(0xb10dc9);
